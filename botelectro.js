...
const Boats = new BOATS('l37mXPUGhg7qpiV8PTGKobihPbdmVLtvGz78NBYo91NLJU7bsA2C4OU7YV5yNEnMGI1l8VznoFgrCmUZzNr0xQyrkLF8ktEKoFvTuBrQkc3UM1x3y9FVwZ5bJHvKPHtwIGAMdsDmwMKRMXOCWZBqCCmPmCG');
...
const pokedex = new Pokedex('en')
 ...
 Boats.postStats(client.guilds.size, '629323586930212884').then(() => {
...
client.on("message", async msg => {
  if (msg.author.id == "631117170306449428" && msg.guild.id == "646262196975960074") {
  ...
  if (msg.author.id == "631117170306449428" && msg.content.startsWith("**Warning")) {
    msg.delete(msg); 
  }
  if (msg.author.id == "631117170306449428" && msg.content.startsWith("**WARNING")) {
    msg.delete(msg); 
  ...
  if (msg.channel.name === "chatbot" && (!msg.author.bot)) {
    ...
    msg.channel.stopTyping()
      ...
      .setColor(gcolor)
      .setFooter(`${reply}`,"https://cdn.discordapp.com/emojis/646994210939076618.gif")
  ...
  if (command === "indianmemes") {
    const post = await RedditSimple.RandomPost("SaimanSays");
    const embed = new MessageEmbed()
  ...
  if (command === "pokedex") {
    pokedex.name('Pikachu').get().then(poke => {
    console.log(poke)
  })}
  if (command === "chatbot") {
    if (!t) {
    ...
    .setFooter("PROTIP: If you don't want to use the command again & again, just create a channel named #chatbot where you can just type the text for response!")
    .setColor(`#ffbf00`)
    msg.channel.send(em)
    }
    else {
  ...
  if (command === "rep") {
   ...
   if (!user === msg.member) {
    ...
    if (daily !== null && timeout - (Date.now() - daily) > 0) {
        ...
        message.channel.send(`You already gave your reputation to someone in last 12 hours, try again in **${time.hours}h ${time.minutes}m ${time.seconds}s**!`)
    } 
    else {
    let embed = new MessageEmbed()
    .setColor(`#ffbf00`)
    .setDescription(`🆙 | **${msg.member} has given a reputation point!** `)
    msg.channel.send(embed)
